...
## Step 2: Prepare Docker Compose (PostgreSQL + API)

Use the provided `backend/docker-compose.yml`, which runs Postgres and the API. It includes:
...
services:
  db:
    image: postgres:16
    environment:
      POSTGRES_DB: parkipal
      POSTGRES_USER: parkipal
  ...
  parkinsons-api:
    ...
    environment:
      ...
      - DB_URL=postgres://parkipal:change-me@db:5432/parkipal
      - ALLOWED_ORIGINS=http://your-frontend-host
    depends_on:
      - db
```
...
- Verify `DB_URL` matches credentials in compose
- Ensure the volume `./data/postgres` is writable
